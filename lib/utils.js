...
module.exports.getContactURL = function(contact) {
  return Link.fromContact(contact).toString();
};
...
module.exports.isValidContact = function(contact, loopback) {
  ...
  const isAllowedAddr = ip.isLoopback(info.hostname) 
    ? !!loopback 
    : true;
...
module.exports.decrypt = function(sk, message) {
  sk = PrivateKey.fromHex(sk);
  return decrypt(sk.secret, message);
};

module.exports.encrypt = function(pk, message) {
  return encrypt(pk, message);
};

module.exports.passwordProtect = function(password, salt, data) {
...
module.exports.passwordUnlock = function(password, salt, data) {
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, 'sha512');
  const iv = key.slice(0, 16);
