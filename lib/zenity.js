...
class Dialog {
    ...
    static notify(msg, title, icon) {
        runAsync(`${zenityBin} --notification --text="${msg}" --title ${title || ''} --window-icon ${icon}`);
    }
    ...
    static textInfo(text, title, options) {
      return new Promise((resolve) => {
        ...
        if(options) {
            ...
            if(options.icon) cmd.push(`--window-icon="${options.icon}"`);
            if(options.parent) cmd.push(`--attach="${options.parent}"`);
            if (options.checkbox) cmd.push(`--checkbox=${options.checkbox}`);
        }
        cmd.push(`--title=${title||"title"}`);
    ...
    static progress(text,title,options) {
        ...
        if(options) {
            ...
            if(options.icon) cmd.push(`--window-icon="${options.icon}"`);
            if(options.parent) cmd.push(`--attach="${options.parent}"`);
            if(options.pulsate) cmd.push(`--pulsate`);
            if(options.autoKill) cmd.push(`--auto-kill`);
            if(options.noCancel) cmd.push(`--no-cancel`);
        }
        cmd.push(`--text="${text||"text"}"`);
        cmd.push(`--title="${title||"title"}"`);
