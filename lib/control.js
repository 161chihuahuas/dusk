...
class Control {
  ...
  getinfo(callback) {
    ...
    callback(null, {
      ...
      peers,
      webdav: {
        onion: this.node.webDavHsAddr,
        local: this.node.webDavLocalAddr 
      },
      drpbox: {
        onion: this.node.anonDrpHsAddr,
        local: this.node.anonDrpLocalAddr
      },
      proxy: 'socks://127.0.0.1:' + this.node.onion.socksPort
    });
  }
  /**
  ...
  subscribe(hexKey, onionAddrHook, callback) {
    if (arguments.length !== 3) {
      return arguments[arguments.length - 1](new Error('Invalid params'));
    }
    this.node.quasarSubscribe(hexKey, (contents, topic) => { 
      this.node.logger.info('sending onion hook for topic %s', topic);
      const agent = this.node.onion.createSocksAgent();
      const payload = JSON.stringify({ contents, topic });
      const webhook = http.request('http://' + onionAddrHook, {
        agent,
        method: 'POST',
        headers: {
        ...
        resp.on('error', err => {
          this.node.logger.error('onion hook response error, %s', err.message);
        });
      });
      webhook.on('error', err => {
        this.node.logger.error('onion hook failed, %s', err.message);
      });
      webhook.write(payload);
      webhook.end();
    });
    callback(null, {
      topic: hexKey,
  ...
  getlocal(hexKey, callback) {
    if (arguments.length !== 2) {
      return arguments[arguments.length - 1](new Error('Invalid params'));
    }

    this.node.localGet(hexKey).then(
      (val) => callback(null, val.toString('hex')),
      (err) => callback(err)
    );
  }

  putlocal(hexValue, callback) {
    if (arguments.length !== 2) {
      return arguments[arguments.length - 1](new Error('Invalid params'));
    }

    this.node.localPut(Buffer.from(hexValue, 'hex')).then(
