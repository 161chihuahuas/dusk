...
class KademliaNode extends AbstractNode {
  ...
  _refresh(startIndex) {
    ...
    return new Promise((resolve, reject) => {
      async.eachSeries(utils.knuthShuffle(indices), (index, next) => {
  ...
  /**
   * Proxy get() to the local storage layer
   * @param {string} key
   * @returns Promise<Buffer>
   */
  localGet(key) {
    return new Promise((resolve, reject) => {
      this.storage.get(key, { valueEncoding: 'json' }, (err, entry) => {
        if (err) { 
          return reject(err);
        }
       
        resolve(Buffer.from(entry.value, 'hex'));
      })
    });
  }

  /**
   * Proxy put() to the local storage layer
   * @param {buffer} value
   * @returns Promise<String>
   */
  localPut(value) {
    return new Promise((resolve, reject) => {
      const key = utils.hash160(value).toString('hex'); 
      this.storage.put(
        key,
        this._createStorageItem(value.toString('hex')), 
        { valueEncoding: 'json' }, 
