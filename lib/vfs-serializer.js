    ...
    const _bufferToLink = (path, buffer) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        shredder.stdout.on('data', data => {
          ...
          this.log.info('created new link for %s: %s', filename, result.toString());
        });
          ...
          if (content.length && type.isFile) {
            ...
            try {
              ...
              resources[path].content = [
                (await _bufferToLink(path, Buffer.concat(content))).toJSON().data
              ];
        ...
        if (data.resources) {
          ...
          for (let path in data.resources) {
            ...
            if (data.resources[path].type.isFile) {
              ...
              const link = Buffer.from(data.resources[path].content[0]).toString();
              
              this.log.info('creating virtual resource for %s from resource', link);
            ...
            fs.resources[path] = new VirtualFileSystemResource(data.resources[path]);
          }
        } else {
