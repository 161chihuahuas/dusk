...
class VirtualFS extends FileSystem {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      writeStream.on('close', async () => {
        ...
        try {
          ...
          await fs.writeFile(path.join(bundleDir, `${metaHash160}.meta`), metaEnc);
        } catch (err) {
          console.log(err)
          writeStream.end();
  ...
  read(fileName) {
    return new Promise(async (resolve, reject) => {
          ...
          while (!success) {
            try {
              ...
              if (missingPieces > metaData.p) {
                return reject(new Error('too many missing shards to recover this file'));
              }
      ...
      if (missingPieces) {
        shards = splitSync(await reedsol.encodeCorrupted(splitSync(Buffer.concat(shards), {
      ...
      const decryptedFile = utils.decrypt(this._sk.toString('hex'), mergedNormalized);
