...
class VirtualFS extends FileSystem {
  ...
  _walkVirtualTree(toPath) {
    ...
    for (let i = 0; i < splitPath.length; i++) {
      if (!splitPath[i]) {
        ...
        throw new Error(`Invalid path ${toPath} ${splitPath}`);
      }
    }
  ...
  delete(delPath) {
    return new Promise(async (resolve, reject) => {
      ...
      if (cursor.type === 'dir') {
        await fs.rmdir(path.join(this.root, delPath), { recursive: true });
      } else if (cursor.type === 'bundle') {
        await fs.rmdir(path.join(this.root, delPath + '.duskbundle'), { recursive: true });
      }
    });
  }
